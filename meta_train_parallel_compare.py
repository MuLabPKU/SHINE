...
class NewSquadCollator:
    def __init__(self, tokenizer):
        self.pad_token_id = tokenizer.pad_token_id

    def __call__(self, batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        return {
            "input_ids": pad_sequence([b["input_ids"] for b in batch], batch_first=True, padding_value=self.pad_token_id),
            "input_attention_mask": pad_sequence([b["input_attention_mask"] for b in batch], batch_first=True, padding_value=0),
            "evidence_ids": pad_sequence([b["evidence_ids"] for b in batch], batch_first=True, padding_value=self.pad_token_id),
            "evidence_attention_mask": pad_sequence([b["evidence_attention_mask"] for b in batch], batch_first=True, padding_value=0),
            "labels": pad_sequence([b["labels"] for b in batch], batch_first=True, padding_value=-100),
        }
        

class NewTextDataset(Dataset):
    def __init__(self, cache_path: str, max_entry: int = -1):
        self.data = []
        with open(cache_path, "r") as f:
            for i, l in enumerate(tqdm(f, desc="Loading...")):
                if max_entry > 0 and i >= max_entry:
                    break
                entry = json.loads(l)
                input_ids = torch.tensor(entry["input_ids"], dtype=torch.long)
                input_attention_mask = torch.ones_like(input_ids)
                evidence_ids = torch.tensor(entry["evidence_ids"], dtype=torch.long)
                evidence_attention_mask = torch.ones_like(evidence_ids)
                label_pos = entry["label_pos"]
                labels = input_ids.clone()
                labels[:label_pos] = -100
                self.data.append({
                    "input_ids": input_ids,
...
def main(cfg: DictConfig):
    ...
    if cfg.data.source == "transmla":
        train_ds = NewTextDataset("data/transmla_tokenized/train.jsonl", max_entry=10000)
        val_ds = NewTextDataset("data/transmla_tokenized/val.jsonl")
        collator = NewSquadCollator(tokenizer)
